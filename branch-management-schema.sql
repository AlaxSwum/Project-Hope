-- ===========================================
-- PHARMACY BRANCHES MANAGEMENT SYSTEM
-- Hope Pharmacy IMS - Branch Management Schema
-- ===========================================
...
ALTER TABLE checklist_folders ADD COLUMN IF NOT EXISTS branch_id UUID REFERENCES pharmacy_branches(id) ON DELETE CASCADE;
ALTER TABLE checklists ADD COLUMN IF NOT EXISTS branch_id UUID REFERENCES pharmacy_branches(id) ON DELETE CASCADE;

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_pharmacy_branches_active ON pharmacy_branches(is_active);
CREATE INDEX IF NOT EXISTS idx_pharmacy_branches_branch_code ON pharmacy_branches(branch_code);
CREATE INDEX IF NOT EXISTS idx_pharmacy_branches_manager ON pharmacy_branches(branch_manager_id);
CREATE INDEX IF NOT EXISTS idx_pharmacy_branches_city ON pharmacy_branches(city);
...
CREATE INDEX IF NOT EXISTS idx_user_branch_assignments_active ON user_branch_assignments(is_active);
CREATE INDEX IF NOT EXISTS idx_user_branch_assignments_primary ON user_branch_assignments(is_primary_branch);

CREATE INDEX IF NOT EXISTS idx_checklist_folders_branch ON checklist_folders(branch_id);
CREATE INDEX IF NOT EXISTS idx_checklists_branch ON checklists(branch_id);
...
CREATE OR REPLACE FUNCTION get_branch_staff_count(branch_uuid UUID)
RETURNS INTEGER AS $$
BEGIN
    RETURN (
...
CREATE OR REPLACE FUNCTION get_branch_performance_summary(branch_uuid UUID, date_for DATE DEFAULT CURRENT_DATE)
RETURNS JSONB AS $$
DECLARE
    result JSONB;
BEGIN
...
-- Remove any existing sample/dummy data
